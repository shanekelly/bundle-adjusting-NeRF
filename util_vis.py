...
def dump_images(opt, idx, name, images, masks=None, from_range=(0, 1), cmap="gray"):
    images = preprocess_vis_image(opt, images, masks=masks,
                                  from_range=from_range, cmap=cmap)  # [B,3,H,W]
...
def color_border(images, colors, width=3):
    ...
    for i, image in enumerate(images):
        image_pad = torch.ones(3, image.shape[1]+width*2,
                               image.shape[2]+width*2)*(colors[i, :, None, None]/255.0)
...
def plot_save_poses(opt, fig, pose, pose_ref=None, path=None, ep=None):
    ...
    cam_depth = opt.visdom.cam_depth if opt.visdom else 0.2
