...
class Model():
    ...
    def load_dataset(self, opt, eval_split="val"):
        ...
        self.train_data = data.Dataset(
            opt, split="train", subset=opt.data.train_sub, downsample=opt.data.train_downsample)
        self.train_loader = self.train_data.setup_loader(opt, shuffle=True)
        log.info("loading test data...")
        if opt.data.val_on_test:
            eval_split = "test"
        self.test_data = data.Dataset(
            opt, split=eval_split, subset=opt.data.val_sub, downsample=opt.data.val_downsample)
    ...
    def train_epoch(self, opt):
        ...
        loader = tqdm.tqdm(self.train_loader,
                           desc="training epoch {}".format(self.ep+1), leave=False)
        for batch in loader:
            # train iteration
            var = edict(batch)
