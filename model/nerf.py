...
class Model(base.Model):
        ...
        if opt.tb:
            ...
            if not opt.nerf.rand_rays or split != "train":
                ...
                if opt.nerf.fine_sampling:
                    ...
                    rgb_map = var.rgb_fine.view(-1, opt.H, opt.W,
    ...
    def generate_videos_synthesis(self, opt, eps=1e-10):
        ...
        if opt.data.dataset in ["blender"]:
            ...
            os.system(
                "ffmpeg -y -framerate 30 -i {0}/rgb_%d.png -pix_fmt yuv420p {1} >/dev/null 2>&1".format(test_path, rgb_vid_fname))
            os.system(
                "ffmpeg -y -framerate 30 -i {0}/depth_%d.png -pix_fmt yuv420p {1} >/dev/null 2>&1".format(test_path, depth_vid_fname))
        else:
            ...
            for i, pose in enumerate(pose_novel_tqdm):
                ...
                invdepth = (1-ret.depth)/ret.opacity if opt.camera.ndc else 1 / \
                    (ret.depth/ret.opacity+eps)
...
class Graph(base.Graph):
        ...
        if opt.nerf.fine_sampling:
            with torch.no_grad():
                ...
                depth_samples_fine = self.sample_depth_from_pdf(
                    opt, pdf=prob[..., 0])  # [B,HW,Nf,1]
                depth_samples = torch.cat(
                    [depth_samples, depth_samples_fine], dim=2)  # [B,HW,N+Nf,1]
            ...
            rgb_fine, depth_fine, opacity_fine, _ = self.nerf_fine.composite(
                opt, ray, rgb_samples, density_samples, depth_samples)
            ret.update(rgb_fine=rgb_fine, depth_fine=depth_fine,
                       opacity_fine=opacity_fine)  # [B,HW,K]
        return ret
    ...
    def sample_depth_from_pdf(self, opt, pdf):
        ...
        depth_bin = torch.linspace(
            depth_min, depth_max, opt.nerf.sample_intvs+1, device=opt.device)  # [N+1]
        depth_bin = depth_bin.repeat(*cdf.shape[:-1], 1)  # [B,HW,N+1]
        depth_low = depth_bin.gather(dim=2, index=(idx-1).clamp(min=0))  # [B,HW,Nf]
        depth_high = depth_bin.gather(dim=2, index=idx.clamp(
            max=opt.nerf.sample_intvs))  # [B,HW,Nf]
...
class NeRF(torch.nn.Module):
    ...
    def forward_samples(self, opt, center, ray, depth_samples, mode=None):
        ...
        rgb_samples, density_samples = self.forward(
            opt, points_3D_samples, ray_unit=ray_unit_samples, mode=mode)  # [B,HW,N],[B,HW,N,3]
