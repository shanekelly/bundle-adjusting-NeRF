...
class Model(nerf.Model):
        ...
        if opt.optim.sched_pose:
            ...
            if opt.optim.lr_pose_end:
                ...
                opt.optim.sched_pose.gamma = (
                    opt.optim.lr_pose_end/opt.optim.lr_pose)**(1./opt.max_iter)
    ...
    def log_scalars(self, opt, var, loss, metric=None, step=0, split="train"):
        ...
        # compute pose error
        if split == "train" and opt.data.dataset in ["blender", "llff", "bonn"]:
    ...
    def evaluate_test_time_photometric_optim(self, opt, var):
        ...
        iterator = tqdm.trange(opt.optim.test_iter, desc="test-time optim.",
    ...
    def generate_videos_pose(self, opt):
        self.graph.eval()
        fig = plt.figure(figsize=(10, 10) if opt.data.dataset in ["blender", "bonn"] else (16, 8))
